...
function initAnimations(){
    ...
    tt
        ...
        .from('#cta',0.7, {opacity:0, scale:0.5,transformOrigin: "60% 50%", ease: Back.easeOut.config(1.7),  onComplete:actionsButton}, "-=0.2");
...
function actionsButton(){
    _btnExit.addEventListener('mouseover', function(){
        ...
        TweenMax.to('#cta',.5,{scale:1.04, transformOrigin: "60% 50%", ease: Elastic.easeOut.config(1.2, 0.4)});
    });
    _btnExit.addEventListener('mouseout', function(){
       
        TweenMax.to('#cta',.5,{scale:1, transformOrigin: "60% 50%", ease: Elastic.easeOut.config(1.2, 0.4)});
    });
